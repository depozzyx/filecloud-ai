    ...
    <div className="relative">
      ...
      >
        {assets.map((asset, index) => (
          <div key={asset.id} className="relative h-[520px] w-full flex-none snap-center bg-black">
            <SlideMedia asset={asset} index={index} />

            <div className="absolute left-2 top-2 rounded-full bg-black/60 px-2 py-1 text-[12px] text-white">
              {index === activeIndex ? activeBadge : BADGE_LABELS[asset.kind]}
            </div>
              ...
              <div className="absolute bottom-3 left-0 right-0 flex items-center justify-center gap-1.5">
                {assets.map((_, dotIndex) => (
                  <button
                    ...
                    className={`h-1.5 w-1.5 rounded-full ${dotIndex === activeIndex ? "bg-white" : "bg-white/50"}`}
